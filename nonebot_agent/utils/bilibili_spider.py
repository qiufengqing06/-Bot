...
class BilibiliDownloader:
    def __init__(self, base_dir="../../data/videos"):
        ...
        co.mute(True)
        co.set_user_data_path(os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'bili_browser_data'))
    ...
    def download(self, url):
        ...
        核心调用方法
        :param url: B站视频链接
        :return: dict {status, message, file_path, title}
        """
        result = {
            "status": "failed",
            "message": "",
        ...
        try:
            ...
            if not playinfo_match:
                result["message"] = "未找到视频流信息 (__playinfo__)"
                logging.error(result["message"])
                return result

            play_info = json.loads(playinfo_match.group(1))

            # 获取 dash 格式流媒体 (通常包含 video 和 audio 数组)
            dash_data = play_info.get('data', {}).get('dash', {})

            if not dash_data:
                result["message"] = "未找到 DASH 流媒体数据"
                logging.error(result["message"])
                return result
            ...
            if os.path.exists(final_path):
                logging.info(f"文件已存在: {final_path}")
                result.update({
                    "status": "success",
                    "message": "文件已存在",
            ...
            with self.session.get(video_url, headers=headers, stream=True, timeout=60) as r:
                r.raise_for_status()
            ...
            with self.session.get(audio_url, headers=headers, stream=True, timeout=60) as r:
                r.raise_for_status()
            ...
            # 5. 合并
            if self.merge_video_audio(v_path, a_path, final_path):
                result.update({
                    "status": "success",
                    "message": "下载完成",
                    "file_path": os.path.abspath(final_path),
